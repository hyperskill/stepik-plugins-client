...
def truncate_data(value: Any, max_length: int = 100) -> Any:
    ...
    if isinstance(value, tuple | list):
        return type(value)([truncate_data(v, max_length=max_length) for v in value])

    if isinstance(value, dict):
        return type(value)((k, truncate_data(v, max_length=max_length)) for k, v in value.items())
